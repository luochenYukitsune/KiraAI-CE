...
(function() {
    ...
    const ColorSchemeManager = {
        ...
        applyScheme(id) {
            ...
            const isDarkTheme = id === 'preset-dark' || 
                               (scheme.colors && this.isDarkColor(scheme.colors.pageBgStart));
        ...
        applyColors(colors) {
            ...
            const specialMappings = {
                aeroOpacity: { var: '--aero-opacity', transform: (v) => v },
                blurIntensity: { var: '--blur-intensity', transform: (v) => v + 'px' }
            };
            
            Object.entries(colors).forEach(([key, value]) => {
                if (specialMappings[key]) {
                    const mapping = specialMappings[key];
                    root.style.setProperty(mapping.var, mapping.transform(value));
                } else {
        ...
        applyCurrentScheme() {
            ...
            if (scheme) {
                const isDarkTheme = scheme.id === 'preset-dark' || 
                                   (scheme.colors && this.isDarkColor(scheme.colors.pageBgStart));
